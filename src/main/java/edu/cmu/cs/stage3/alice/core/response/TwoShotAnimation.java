...
public class TwoShotAnimation extends AbstractPositionAnimation {
	public final SpatialRelationProperty spatialRelation = new SpatialRelationProperty(this, "spatialRelation",
			edu.cmu.cs.stage3.alice.core.SpatialRelation.RIGHT_OF);
	...
	private final double angleToRotate = java.lang.Math.PI / 6; // 5 degrees,
																// for now
	...
	public class RuntimeTwoShotAnimation extends RuntimeAbstractPositionAnimation {
		...
		public void prologue(final double t) {
			...
			if (m_asSeenBy == null) {
				throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException(
						"first character value must not be null.", null, TwoShotAnimation.this.asSeenBy);
			}
			...
			if (m_asSeenBy2 == null) {
				throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException(
						"second character value must not be null.", getCurrentStack(), asSeenBy2);
			}
			if (m_subject == null) {
				throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException("subject value must not be null.",
						getCurrentStack(), TwoShotAnimation.this.subject);
			}
			if (m_asSeenBy2 == m_asSeenBy) {
				throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException(
						"first and second characters must be different", getCurrentStack(), asSeenBy2);
			}
			if (m_subject == m_asSeenBy || m_subject == m_asSeenBy2) {
				throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException(
						"subject value cannot be the same as either character", getCurrentStack(),
						TwoShotAnimation.this.subject);
			}
			if (m_amount < 0) {
				throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException("amount must be greater than 0",
						getCurrentStack(), amount);
			}
			// CLK - end
		...
		protected edu.cmu.cs.stage3.math.Box getBoundingBox(final ReferenceFrame ref, final ReferenceFrame asSeenBy) {
		...
		protected double calculateB(final double x0, final double z0, final double dx, final double dz) {
			return z0 - dz / dx * x0;
		}
		...
		protected double calculateXIntersect(final double dxLeft, final double dzLeft, final double bLeft,
				final double bForward) {
		...
		// on intersection with
		// a bounding box min or max)
		protected double calculateZIntersect(final double dxLeft, final double dzLeft, final double bLeft,
				final double bForward) {
			return (dxLeft / dzLeft * bLeft + dzLeft / dxLeft * bForward) / (dxLeft / dzLeft + dzLeft / dxLeft);
		}

		protected double calculateDistance(final double x1, final double z1, final double x2, final double z2) {
		...
		protected javax.vecmath.Vector3d getPositionEnd() {
			...
			final edu.cmu.cs.stage3.math.Matrix33 cameraEndOrientation = m_asSeenBy.getOrientationAsAxes(m_asSeenBy);
			...
			if (sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.LEFT_OF)) {
				...
				cameraEndOrientation.setForwardUpGuide(new javax.vecmath.Vector3d(1, 0, 0),
						cameraEndOrientation.getRow(1));
				...
				cameraEndPos = edu.cmu.cs.stage3.alice.core.SpatialRelation.BEHIND.getPlaceVector(1,
						m_subjectBoundingBox, m_asSeenByBoundingBox);
				cameraEndOrientation.setForwardUpGuide(new javax.vecmath.Vector3d(0, 0, 1),
						cameraEndOrientation.getRow(1));
				...
				cameraEndPos = edu.cmu.cs.stage3.alice.core.SpatialRelation.IN_FRONT_OF.getPlaceVector(1,
						m_subjectBoundingBox, m_asSeenByBoundingBox);
				cameraEndOrientation.setForwardUpGuide(new javax.vecmath.Vector3d(0, 0, -1),
						cameraEndOrientation.getRow(1));
				cameraEndOrientation.rotateY(angleToRotate);
				...
				cameraEndPos = sv.getPlaceVector(1, m_subjectBoundingBox, m_asSeenByBoundingBox);
				cameraEndOrientation.setForwardUpGuide(new javax.vecmath.Vector3d(0, -1, 0),
						new javax.vecmath.Vector3d(-1, 0, 0));
			...
			if (sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.BEHIND_LEFT_OF)
					|| sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.BEHIND_RIGHT_OF)
					|| sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.FRONT_LEFT_OF)
					|| sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.FRONT_RIGHT_OF)) {
				// if
				// (!sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.ABOVE)
				// &&
				// !sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.BELOW))
				// {
				final javax.vecmath.Vector3d leftVector = cameraEndOrientation.getRow(0);
				final javax.vecmath.Vector3d forwardVector = cameraEndOrientation.getRow(2);

				final double bLeft = calculateB(cameraEndPos.x, cameraEndPos.z, leftVector.x, leftVector.z);

				// min bounds for bounding box
				final javax.vecmath.Vector3d boxMin = m_asSeenByBoundingBox.getMinimum();
				double bForward = calculateB(boxMin.x, boxMin.z, forwardVector.x, forwardVector.z);
				final double minX = calculateXIntersect(leftVector.x, leftVector.z, bLeft, bForward);
				final double minZ = calculateZIntersect(leftVector.x, leftVector.z, bLeft, bForward);

				// max bounds for bounding box
				final javax.vecmath.Vector3d boxMax = m_asSeenByBoundingBox.getMaximum();
				bForward = calculateB(boxMax.x, boxMax.z, forwardVector.x, forwardVector.z);
				final double maxX = calculateXIntersect(leftVector.x, leftVector.z, bLeft, bForward);
				final double maxZ = calculateZIntersect(leftVector.x, leftVector.z, bLeft, bForward);
				...
				final double distance = calculateDistance(minX, minZ, maxX, maxZ);
				...
				final double halfViewable = m_asSeenByBoundingBox.getHeight() * amount.doubleValue() * 1.5 / 2.0;
				final double startingHeight = (1.0 - amount.doubleValue()) * m_asSeenByBoundingBox.getHeight();
				m_cameraHeight = startingHeight + halfViewable + m_asSeenByBoundingBox.getMinimum().y;
				cameraEndPos.y = m_cameraHeight;

			} else if (sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.LEFT_OF)
				...
				cameraEndPos.z = m_asSeenByBoundingBox.getCenter().z;
					...
					|| sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.BEHIND)) {
				...
				final double halfViewable = m_asSeenByBoundingBox.getHeight() * 1.5 * amount.doubleValue() / 2.0;
				final double startingHeight = (1.0 - amount.doubleValue()) * m_asSeenByBoundingBox.getHeight();
				m_cameraHeight = startingHeight + halfViewable + m_asSeenByBoundingBox.getMinimum().y;
				...
				final double halfViewable = m_asSeenByBoundingBox.getDepth() / 2.0;
		...
		public void update(final double t) {
			...
			final edu.cmu.cs.stage3.math.Matrix33 nextOrient = edu.cmu.cs.stage3.math.Matrix33
					.interpolate(m_cameraBeginOrientation, m_cameraEndOrientation, getPortion(t));
